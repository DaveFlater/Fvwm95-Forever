		   ...
		   unsigned long context, char *action, int *Module)
{
  ...
  // 2017-04-08 DWF:  fixed underflow on boundary_width == 0.
  gcv.line_width = (tmp_win->boundary_width>1 ? tmp_win->boundary_width-1 : 1);
  ...
  if (ymotion == -1)
    YOffset = dragHeight - YOffset;
  if (xmotion == -1)
    XOffset = dragWidth - XOffset;
...
void DoResize(int x_root, int y_root, FvwmWindow * tmp_win)
{
  ...
  if ((xmotion == 0) && (ymotion == 0)) {
    ...
    if ((y_root <= origy) || (y_root >= origy + origHeight - 1)) {
      ...
      if (y_root <= origy)
	ymotion = 1;
      if (y_root >= origy + origHeight - 1)
	ymotion = -1;
      if (x_root <= origx + ct)
	xmotion = 1;
      if (x_root >= origx + origWidth - ct - 1)
	xmotion = -1;
	...
	xmotion = -1;
      if (y_root <= origy + ct)
	ymotion = 1;
      if (y_root >= origy + origHeight - ct - 1)
	ymotion = -1;
    }
  ...
  if (Init) {
    ...
    if (Scr.d_depth >= 2)
      RelieveWindow(tmp_win,
		    ...
		    Scr.StdFont.height + SIZE_VINDENT * 2, Scr.WinReliefGC,
		    Scr.WinShadowGC, FULL_HILITE);
  } else {
