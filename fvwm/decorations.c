		 ...
		 int resize_width)
{
  ...
  if (t->functions & MWM_FUNC_ALL) {
    ...
    t->functions = (MWM_FUNC_RESIZE | MWM_FUNC_MOVE | MWM_FUNC_MINIMIZE |
		    MWM_FUNC_MAXIMIZE | MWM_FUNC_CLOSE) & (~(t->
							     functions));
  }
...
int check_allowed_function(MenuItem * mi)
{
  ...
  if ((Tmp_win) &&
      ...
      && (mi->func_type == F_DELETE)) return 0;

  /* Move is a funny hint. Keeps it out of the menu, but you're still allowed
   * to move. */
  if ((mi->func_type == F_MOVE) && (Tmp_win)
      && (!(Tmp_win->functions & MWM_FUNC_MOVE))) return 0;

  if ((mi->func_type == F_RESIZE) && (Tmp_win) &&
      (!(Tmp_win->functions & MWM_FUNC_RESIZE))) return 0;

  if ((mi->func_type == F_ICONIFY) && (Tmp_win) &&
      (!(Tmp_win->flags & ICONIFIED)) &&
      (!(Tmp_win->functions & MWM_FUNC_MINIMIZE))) return 0;

  if ((mi->func_type == F_MAXIMIZE) && (Tmp_win) &&
      (!(Tmp_win->functions & MWM_FUNC_MAXIMIZE))) return 0;

  if ((mi->func_type == F_DELETE) && (Tmp_win) &&
      (!(Tmp_win->functions & MWM_FUNC_CLOSE))) return 0;
  ...
  if (mi->func_type == F_FUNCTION) {
	...
	(strncasecmp(mi->item, RESIZE_STRING1,
		     strlen(RESIZE_STRING1)) == 0)) return 0;

    if ((Tmp_win) && (!(Tmp_win->functions & MWM_FUNC_RESIZE)) &&
	(strncasecmp(mi->item, RESIZE_STRING2,
		     strlen(RESIZE_STRING2)) == 0)) return 0;

    if ((Tmp_win) && (!(Tmp_win->functions & MWM_FUNC_MINIMIZE)) &&
	(!(Tmp_win->flags & ICONIFIED)) &&
	(strncasecmp(mi->item, MINIMIZE_STRING, strlen(MINIMIZE_STRING)) ==
	 0)) return 0;

    if ((Tmp_win) && (!(Tmp_win->functions & MWM_FUNC_MINIMIZE)) &&
	(strncasecmp(mi->item, MINIMIZE_STRING2,
		     strlen(MINIMIZE_STRING2)) == 0))
      return 0;

    if ((Tmp_win) && (!(Tmp_win->functions & MWM_FUNC_MAXIMIZE)) &&
	(strncasecmp(mi->item, MAXIMIZE_STRING,
		     strlen(MAXIMIZE_STRING)) == 0)) return 0;
...
int check_allowed_function2(int function, FvwmWindow * t)
{
  ...
  if ((function == F_MAXIMIZE) && (t) &&
      (!(t->functions & MWM_FUNC_MAXIMIZE))) return 0;
