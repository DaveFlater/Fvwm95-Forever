...
void free_icon_name(FvwmWindow *tmp)
{
  ...
  if (tmp->icon_name != NULL &&
      ...
      tmp->icon_name != tmp->class.res_class) XFree(tmp->icon_name);
...
void free_window_name(FvwmWindow *tmp)
{
  ...
  if (tmp->name != NULL &&
      ...
      tmp->name != tmp->class.res_class) XFree(tmp->name);
...
void Destroy(FvwmWindow *Tmp_win)
{
  ...
  /*
     RBW - 05/15/1998 - new: have to unhook the stacking order chain also.
...
Bool StashEventTime(XEvent *ev)
{
  ...
  if ((NewTimestamp > lastTimestamp)
      || ((lastTimestamp - NewTimestamp) > 30000)) lastTimestamp =
	NewTimestamp;
...
void RaiseWindow(FvwmWindow *t)
{
  ...
  count = 1;
  Broadcast(M_RAISE_WINDOW, 3, t->w, t->frame, (unsigned long) t, 0, 0, 0,
	    0);
  ...
  if (i > 0) {
    ...
    /*
       RBW - 05/15/1998 - new: maintain the stacking order chain.
    */
...
void LowerWindow(FvwmWindow *t)
{
  ...
  Broadcast(M_LOWER_WINDOW, 3, t->w, t->frame, (unsigned long) t, 0, 0, 0,
	    0);
...
char *stripcpy(char *source)
{
  ...
  len = strlen(source);
  tmp = source + len - 1;
  // 2017-04-13 DWF  Fixed underflow
...
void fvwm_msg(int type, char *id, char *msg, ...)
{
  ...
  // 2017-03-11 DWF fixed incorrect use of va_start / va_end that corrupted
  // the ERR printout below.
  if (type == ERR) {
    ...
    va_end(args);
    // 2017-03-12 DWF fixed this epic fail too:
    // tmp[strlen(tmp)] = '\n';
    // tmp[strlen(tmp) + 1] = '\0';
    strcat(tmp, "\n");
