		     ...
		     unsigned long context, char *text, int *Module)
{
  ...
  while ((*restofline != 0) && (*restofline != '\n')) {
	...
	while (isspace(*restofline))
	  restofline++;
	// 2017-04-15 DWF  Handled quoted identifiers.
	if (*restofline == '"') {
	  quoted=1;
	  ++restofline;
	}
	...
	while (*tmp &&
	       (quoted && *tmp != '"' && *tmp != '\n' ||
		!quoted && *tmp != ',' && !isspace(*tmp)))
	  ++tmp;
	if ((len = tmp - restofline) > 0) {
	  char *usename;
	  nptr = Scr.TheList;
	  // 2017-04-17 DWF
	  // It was using strncasecmp here, resulting in false matches like
	  // UseStyle log --> LogoTestButtons.
	  usename = strndup(restofline, len);
	  while (nptr && strcasecmp(usename, nptr->name))
	    nptr = nptr->next;
	  if (nptr && !strcasecmp(usename, nptr->name)) {
	    ...
	    restofline = tmp;
	    // 2017-04-15 DWF fixed memory error.
	    tmp = safemalloc(len+1);
	    strncpy(tmp, restofline - len, len);
	    tmp[len] = '\0';
	    fvwm_msg(ERR, "ProcessNewStyle",
		     "UseStyle: `%s' style not found", tmp);
	    free(tmp);
	  }
	  free(usename);
	}
	if (quoted && *restofline == '"')
	  ++restofline;
      }
      break;
    case 'v':
    ...
    // 2017-04-15 DWF  Deleted all redundant instances of the following
    // statement that appeared at the ends of blocks above.
    while (isspace(*restofline))
      restofline++;
    if (*restofline == ',')
      restofline++;
  ...
  AddToList(name, icon_name, ticon_name, off_flags, on_flags, desknumber,
	    bw, nobw, forecolor, backcolor, off_buttons, on_buttons,
	    IconBox, BoxFillMethod);
}
...
void AddToList(char *name,
	       ...
	       unsigned long on_buttons, int *IconBox, int BoxFillMethod)
{
  name_list *nptr, *lastptr = NULL;

  if ((name == NULL)
      || ((off_flags == 0) && (on_flags == 0) && (on_buttons == 0)
