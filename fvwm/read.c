...
/*  fvwm_msg(INFO,piperead?"PipeRead":"Read","action == '%s'",action); */
  ...
  (void)GetNextToken(action, &ofilename);
  if (ofilename == NULL) {
...
/*  fvwm_msg(INFO,piperead?"PipeRead":"Read","trying '%s'",filename); */
  ...
  if (fd == NULL) {
    fvwm_msg(ERR,
	     ...
	     "file '%s' not found in $HOME or " FVWMDIR, ofilename);
  ...
  while (tline != (char *) 0) {
    int l;
    // 2017-03-05 DWF fixed 2 buffer bugs
    while (tline && (l = strlen(line)) < sizeof(line)-1 && l>1 &&
	   line[l - 1] == '\n' && line[l - 2] == '\\') {
...
void SendDataToModule(XEvent * eventp, Window w, FvwmWindow * tmp_win,
		      unsigned long context, char *action, int *Module)
{
  // 2020-03-17 DWF fixed buffer overflows.
  ...
  {
    char *message = safemalloc(strlen(IconPath) + 11);
...
#ifdef XPM
  {
    char *message = safemalloc(strlen(PixmapPath) + 13);
