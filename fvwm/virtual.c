...
/***************************************************************************
 *
 * Check to see if the pointer is on the edge of the screen,
 * and scroll/page if needed
 *
 ***************************************************************************/
		  ...
		  int *delta_x, int *delta_y, Bool Grab)
{
  ...
  *delta_y = 0;
  ...
  if ((Scr.VxMax == 0 ||
      ...
      && (Scr.VyMax == 0
	  || (*yt >= SCROLL_REGION
	      && *yt < Scr.MyDisplayHeight - SCROLL_REGION))) return;
  ...
  // 2017-03-12 DWF:  you missed a spot (the four inner else cases).
  // MoveViewport clamps the limits, but unfortunately resize still uses
  // these deltas.  This caused the bug where resizing near the edge of
  // the virtual desktop caused windows to suddenly expand off the screen.
  if (Scr.Vx + *delta_x < 0) {
...
void initPanFrames()
{
  ...
  Scr.PanFrameLeft.win =
      XCreateWindow(dpy, Scr.Root,
		    ...
		    PAN_FRAME_THICKNESS, Scr.MyDisplayHeight - 2 * PAN_FRAME_THICKNESS, 0,	/* no border */
		    CopyFromParent, InputOnly, CopyFromParent,
		    valuemask, &attributes);

  attributes.cursor = Scr.FvwmCursors[TOP];
  Scr.PanFrameTop.win =
      XCreateWindow(dpy, Scr.Root, 0, 0, Scr.MyDisplayWidth, PAN_FRAME_THICKNESS, 0,	/* no border */
  ...
  Scr.PanFrameRight.win =
      XCreateWindow(dpy, Scr.Root,
		    ...
		    PAN_FRAME_THICKNESS, PAN_FRAME_THICKNESS,
		    Scr.MyDisplayHeight - 2 * PAN_FRAME_THICKNESS, 0,	/* no border */
  ...
  Scr.PanFrameBottom.win =
      XCreateWindow(dpy, Scr.Root,
		    0, Scr.MyDisplayHeight - PAN_FRAME_THICKNESS, Scr.MyDisplayWidth, PAN_FRAME_THICKNESS, 0,	/* no border */
		    CopyFromParent, InputOnly, CopyFromParent,
		    valuemask, &attributes);
...
{
  FvwmWindow *t, *t1;
  int deltax, deltay, txl, txr, tyt, tyb;
  int PageTop, PageLeft, PageBottom, PageRight;

  if (grab)
    XGrabServer(dpy);

  if (newx > Scr.VxMax) newx = Scr.VxMax;
  if (newy > Scr.VyMax) newy = Scr.VyMax;
  if (newx < 0) newx = 0;
  if (newy < 0) newy = 0;
  ...
  */
  ...
  Scr.Vx = newx;
  Scr.Vy = newy;
  Broadcast(M_NEW_PAGE, 5,
	    Scr.Vx, Scr.Vy, Scr.CurrentDesk, Scr.VxMax, Scr.VyMax, 0, 0);

  if ((deltax != 0) || (deltay != 0)) {

    /*
       RBW - 05/15/1998  - new: chase the chain bidirectionally, all at
       once! The idea is to move the windows that are moving out of the
       viewport from the bottom of the stacking order up, to minimize the
       expose-redraw overhead. Windows that will be moving into view will
       be moved top down, for the same reason.
          ...
          if (!((t1->flags & ICONIFIED) && (t1->flags & StickyIcon)) &&
            ...
            if (!(t1->flags & StickyIcon)) {
              ...
              if (!(t1->flags &ICON_UNMAPPED))
                Broadcast(M_ICON_LOCATION, 7, t1->w, t1->frame,
                          (unsigned long) t1,
		      ...
		      unsigned long context, char *action, int *Module)
{
  ...
  (void)GetTwoArguments(action, &val1, &val2, &val1_unit, &val2_unit);
...
void changeDesks(int val1, int val2)
{
  ...
  /*
     RBW - 05/08/1998 - new: chase the chain bidirectionally, unmapping
		       ...
		       unsigned long context, char *action, int *Module)
{
  ...
  (void)GetOneArgument(action, &val1, &val1_unit);
		    ...
		    unsigned long context, char *action, int *Module)
{
  ...
  if ((val1_unit != Scr.MyDisplayWidth) ||
    ...
    fvwm_msg(ERR, "goto_page_func",
	     "GotoPage arguments should be unitless");
  }
