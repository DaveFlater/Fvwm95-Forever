  ...
  *w = e->xany.window;
  ...
  if ((e->type == ButtonPress) && (e->xbutton.subwindow != None) &&
      ((e->xbutton.subwindow == t->w)
       || (e->xbutton.subwindow == t->Parent))) *w = e->xbutton.subwindow;
...
void HandleKeyPress()
{
  ...
  Event.xkey.keycode =
      XKeysymToKeycode(dpy, fixKeycodeToKeysym(dpy, Event.xkey.keycode, 0));
...
void HandleButtonPress()
{
  ...
  if ((Tmp_win) && (Tmp_win->flags & ClickToFocus)
      ...
      ((Event.
	xbutton.
	state & (ControlMask | Mod1Mask | Mod2Mask | Mod3Mask | Mod4Mask |
		 Mod5Mask)) == 0)) {
    if (Tmp_win) {
      SetFocus(Tmp_win->w, Tmp_win, 1);
#ifdef CLICKY_MODE_1
...
void HandleEnterNotify()
{
  ...
  if (XCheckTypedWindowEvent(dpy, ewp->window, LeaveNotify, &d)) {
	...
	(d.xcrossing.detail != NotifyInferior)) return;
  }
...
/* an EnterEvent in one of the PanFrameWindows activates the Paging */
  ...
  if (Scr.ColormapFocus == COLORMAP_FOLLOWS_MOUSE) {
    if ((!(Tmp_win->flags & ICONIFIED))
	&& (Event.xany.window ==
	    Tmp_win->w)) InstallWindowColormaps(Tmp_win);
    else
      InstallWindowColormaps(NULL);
  }
  return;
...
void HandleConfigureRequest()
{
  ...
  if (cre->value_mask & CWY)
    y =
	cre->y - Tmp_win->boundary_width - Tmp_win->title_height -
	Tmp_win->bw;
...
int My_XNextEvent(Display *dpy, XEvent *event)
{
  ...
  for (i = 0; i < npipes; i++, wPipePtr++, rPipePtr++, queuePtr++) {
    if (*rPipePtr >= 0)
      FD_SET(*rPipePtr, &in_fdset);
    if (*queuePtr)
      FD_SET(*wPipePtr, &out_fdset);
  }
...
#ifdef __hpux
  retval =
      select(fd_width, (int *) &in_fdset, (int *) &out_fdset, 0, NULL);
#else
