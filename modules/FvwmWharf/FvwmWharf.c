...
// 2017-04-02 DWF:  Animations are now 1 pixel at a time, and the only
// parameter is this delay (microseconds, /2 on open).
...
/*		    if (Event.xclient.data.l[0]!=DndFile ||
		...
		if(*CurrentWin == Folders[x].win)
		  ...
		  if((Buttons[CurrentButton].action)&&
		    ...
		    {
		      // 2017-03-14 DWF
		      // Replaced broken parsing.
		      i=4;
		      if (DWF_parse_arg(Buttons[CurrentButton].action,&i,&i2)) {
			Buttons[CurrentButton].hangon = strndup (
			  &Buttons[CurrentButton].action[i], i2-i+1);
...
void CloseFolder(int folder)
{
    ...
    XGetGeometry(dpy,win,&junk_win,&x,&y,(unsigned*)&w,(unsigned*)&h,&junk_bd,&junk_depth);
  ...
  if(strncasecmp(tline,"swallow",7)==0 || strncasecmp(tline,"maxswallow",10)==0)
    {
	 ...
	 case Wharf spawns and gobbles up window */
      ...
      // 2017-03-14 DWF
      // They forgot to set i to 10 in case of maxswallow.
      i=(tline[0]=='s'? 7 : 10);

      // 2017-03-14 DWF
      // Replaced broken parsing and string munging.
      // The item after Swallow or MaxSwallow is the expected name/resource
      // of the window to swallow, and the rest is the command to launch said
      // window.
      if (DWF_parse_arg(tline,&i,&i2) && tline[i2]!='\0') {
        actual->hangon = strndup (&tline[i], i2-i+1);
	actual->swallow = 1;
	actual->maxsize = (tline[0]=='m');
	...
	while (tline[i] != '\0' && isspace(tline[i]))
	  ...
	  i2 = i + strlen(&tline[i]);
	  while (isspace(tline[i2]))
	    --i2;

	  // If it's a Module command, mark it as such and do it.
	  // (The documented SwallowModule command DOES NOT EXIST.)
	  // Otherwise, prefix the command by Exec and do that.
	  ptr = safemalloc(i2-i+7);
	  if(strncasecmp(&tline[i],"Module",6)==0) {
	    ptr[0] = 0;
	    actual->module = 1;
	  } else
	    strcpy(ptr,"Exec ");
	  strncat (ptr, &tline[i], i2-i+1);
	  SendText(fd,ptr,0);
	  free(ptr);
	  actual->action = NULL;
	}
 ...
 *
 *************************************************************************/
// 2017-03-12 DWF changed signature to pass entire header so that buffer
// overflow in CheckForHangon can be avoided.  (May be redundant, as the
// main problem was lack of breaks in preceding cases.)
      ...
      for (m = 0; m < 8; m++)
        {
          for (i = 0; i < mm->max_keypermod; i++)
            {
      	if ((kc = *kp++) &&
      	    ((ks = fixKeycodeToKeysym(dpy, kc, 0)) != NoSymbol))
      	  {
