	       ...
	       int alloc_relief, int incr)
{
	...
	/* color allocation saver */
        // 2017-03-18 DWF
        // They're trying to convert the range of unsigned short into a
        // signed short.  Float-to-short overflow is undefined.  I was
        // getting the wrong answer on 32 bit, and
        // -fsanitize=float-cast-overflow failed to catch this.
        //   Given float rv = 62965.0
        //   64-bit: (short)rv = -2571
        //   32-bit: (short)rv = -32768
        // Original code:
	// red = ((short)rv) & RED_MASK;
        // The following is what they and I did elsewhere with success.
	red   = ((unsigned short)rv) & RED_MASK;
	green = ((unsigned short)gv) & GREEN_MASK;
	blue  = ((unsigned short)bv) & BLUE_MASK;
	// 2017-03-18 DWF  This deduping logic is also broken since
	// XAllocColor modifies color to equal the color actually allocated.
	// Not worth fixing.
		     ...
		     unsigned long *dark, unsigned long *light)
{
	...
	// 2017-03-18 DWF fixed short --> unsigned short again.
	color.red   = (unsigned short)(rv<0?0:rv);
	color.green = (unsigned short)(gv<0?0:gv);
	color.blue  = (unsigned short)(bv<0?0:bv);
	if (XAllocColor(dpy, DefaultColormap(dpy, DefaultScreen(dpy)),&color))
