...
#define gcm CheckLockModulegcm
  ...
  if (mif->visible && mif->show) {
    // 2017-04-19 DWF  Harmonized cleared area with drawn area (+3).
    XClearArea(dpy, win, win_width-stwin_width+icons_offset+3, 1,
...
int CheckLockModuleSeeMouse(struct MyInfo *mif, int x, int y)
{
  ...
  xr = xcal + win_width - stwin_width + mif->offset + mif->icon_attr.width;
