...
void AddHandle(char *plugin)
{
  ...
  if (Handles[HandlesFree] == NULL) {
    fprintf (stderr, "FvwmTaskBar AddHandle: dlopen failed for %s: %s\n", plugin,
	     dlerror());
    return;
  }
  HandlesFree++;
...
void LoadPlugins(void)
{
  ...
  dir = opendir(plugins);
  if (dir == NULL) {
    fprintf (stderr, "FvwmTaskBar LoadPlugins: failed to opendir %s\n", plugins);
    perror("opendir");
    return;
  }
  ...
  while ((dp = readdir(dir)) != NULL) {
    strcat(strcpy(path, plugins), "/");
    AddHandle(strcat(path, dp->d_name));
  }
  if (errno)
    perror ("FvwmTaskBar LoadPlugins: readdir");
  (void) closedir(dir);
}
...
*/
  ...
  // DWF 2014-01-25
  // When BatStatModule_Init returned NULL because /dev/apm didn't exist,
  // it was crashing FvwmTaskBar.  To avoid this, act like the module
  // didn't load.
  if (GLI[k]->data == NULL)
    GL[k] = NULL;
}
...
int ParseGLinfo(char *tline, char *Module, int Clength)
{
  ...
  // 2017-04-01 DWF added simple check for ID-before-Symbol to try to prevent
  // badness.
  static Bool idcheck=1;

  // 2017-04-01 DWF removed redundant, leaked calloc.
  // Note that CopyString is not just strdup: it strips whitespace too.
  char *s = NULL;
  ...
  if (strncasecmp(tline,CatString3(Module, "GoodyLoadablePlugins",""),
    ...
    return 1;
			 ...
			 Clength+15)==0) {
    // If you forgot GoodyLoadableSymbol, this clobbers the previous one.
    if (idcheck) {
      fprintf (stderr, "Fvwm95: missing *FvwmTaskBarGoodyLoadableSymbol before %s\n", tline);
      return 0;
    }
    ...
    return 1;
			 ...
			 Clength+19)==0) {
    // This is what allocates another handle.
