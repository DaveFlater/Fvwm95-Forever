...
void dateDraw(struct DateInfo *v,Display *dpy,Drawable win) {
  ...
  // 2017-04-19 DWF  Harmonized cleared area with drawn area (+4).
  XClearArea(dpy, win, win_width-stwin_width+icons_offset+4, 1, v->clock_width, RowHeight-2, False);
...
int dateInMouse(struct DateInfo *v,int x, int y) {
  ...
  // for whatever reason.
  const int xcal = 4;
  // In addition, fudge the right margin because font metrics are complex.
  const int xfudge = 3;
  int clockl = xcal + win_width - stwin_width + v->my_offset;
  int clockr = xcal + win_width - stwin_width + v->my_offset + v->clock_width
    - xfudge;
...
void dateTipWindow(struct DateInfo *v) {
  ...
  // 2017-04-14 DWF  Made x coordinate like the other goodies.
  PopupTipWindow(win_width-stwin_width+v->my_offset, 0, str);
}
