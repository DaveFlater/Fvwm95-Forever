...
static void resize_manager (WinManager *man, int w, int h)
{
  XSizeHints size;
  long mask, oldheight;
  long __attribute__((unused)) oldwidth;
...
void xevent_loop (void)
{
  ...
  while (XPending (theDisplay)) {
    ...
    case KeyPress:
      (void)XLookupString ((XKeyEvent *)&theEvent, buffer, 100,
			   &keysym, &compose);
...
void init_window (int man_id)
{
  ...
  if (man->geometry) {
    int gravity, width, height;
    (void)XWMGeometry (theDisplay, theScreen, man->geometry, "+0+0", 1,
