  ...
  if(b!=UberButton)
    {
      ...
      fprintf(stderr,"0x%016lx(%ix%i@(%i,%i),0x%04x): ",
	      (long unsigned)b,b->BWidth,b->BHeight,
	      buttonXPos(b,button),buttonYPos(b,button),b->flags);
    }
  else
    fprintf(stderr,"0x%016lx(%ix%i@,0x%04x): ",(long unsigned)b,
	    b->BWidth,b->BHeight,b->flags);

  if(b->flags&b_Font)
    fprintf(stderr,"Font(%s,%016lx) ",b->font_string,(unsigned long)b->font);
...
*/
	...
	fprintf(stderr,"0x%016lx ",(unsigned long)b->c->buttons[i++]);
      fprintf(stderr,"}\n");
      i=0;
      while(i<b->c->num_buttons)
