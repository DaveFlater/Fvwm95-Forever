...
void ProcessMsgSwallow(struct XObj *xobj,unsigned long type,unsigned long *body)
{
 switch(type)
 {
  case M_MAP:
   swallow(xobj,body);
   break;
  // 2017-04-08 DWF Removed breaks after M_RES_NAME and M_RES_CLASS so that
  // SwallowExec will work with these other identifiers.  C.f. the analogous
  // code in FvwmButtons.c and FvwmWharf.c.
  case M_RES_NAME:
  case M_RES_CLASS:
  case M_WINDOW_NAME:
