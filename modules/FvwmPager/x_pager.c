...
void initialize_pager(void)
{
  ...
  if (Rows < 0) {
    if (Columns < 0) {
      ...
      Rows = ndesks / Columns;
      if (Rows * Columns < ndesks) Rows++;
    }
  }

  if (Columns < 0) {
    Columns = ndesks / Rows;
    if (Rows * Columns < ndesks) Columns++;
  }

  if (Rows * Columns < ndesks) Rows = ndesks / Columns;
  ...
  if (usposition) sizehints.flags |= USPosition;
  ...
  for (i = 0; i < ndesks; i++) {
    ...
    Desks[i].CPagerWin =
	XCreateWindow(dpy, Desks[i].w, -1000, -1000, w, h, 0,
...
void ReConfigureIcons(void)
{
  ...
  while (t != NULL) {
    ...
    x = (Scr.Vx + t->x) * (icon_w - n) /
        (Scr.VxMax + Scr.MyDisplayWidth) + n1;
    y = (Scr.Vy + t->y) * (icon_h - m) /
        (Scr.VyMax + Scr.MyDisplayHeight) + m1;
...
void DrawIconGrid(int erase)
{
  ...
  if (erase) XClearWindow(dpy, icon_win);
...
void AddNewWindow(PagerWindow *t)
{
  ...
  n1 = (Scr.Vx + t->x) / Scr.MyDisplayWidth;
  m1 = (Scr.Vy + t->y) / Scr.MyDisplayHeight;
  x = (Scr.Vx + t->x) * (desk_w - n) /
      (Scr.VxMax + Scr.MyDisplayWidth) + n1;
  y = (Scr.Vy + t->y) * (desk_h - m) /
      (Scr.VyMax + Scr.MyDisplayHeight) + m1;
...
void Scroll(int Desk, int x, int y)
{
  ...
  if (Wait == 0) {
    ...
    if (x > desk_w) x = desk_w;
    if (y > desk_h) y = desk_h;

    sx = (100 * (x * (Scr.VxMax + Scr.MyDisplayWidth) / desk_w - Scr.Vx)) /
	 Scr.MyDisplayWidth;
    sy = (100 * (y * (Scr.VyMax + Scr.MyDisplayHeight) / desk_h - Scr.Vy)) /
	 Scr.MyDisplayHeight;
...
void IconScroll(int x, int y)
{
  ...
  if (Wait == 0) {
    ...
    if (x > icon_w) x = icon_w;
    if (y > icon_h) y = icon_h;

    sx = (100 * (x * (Scr.VxMax + Scr.MyDisplayWidth) / icon_w - Scr.Vx)) /
	 Scr.MyDisplayWidth;
    sy = (100 * (y * (Scr.VyMax + Scr.MyDisplayHeight) / icon_h - Scr.Vy)) /
	 Scr.MyDisplayHeight;
...
{
  char command[100];
  int x1, y1, finished = 0, wx, wy, n, x, y, xi = 0, yi =
      0, wx1, wy1, x2, y2;
  Window dumwin;
  PagerWindow *t;
  int m, n1, m1;
  ...
  n1 = (Scr.Vx + t->x) / Scr.MyDisplayWidth;
  m1 = (Scr.Vy + t->y) / Scr.MyDisplayHeight;
  wx = (Scr.Vx + t->x) * (desk_w - n) /
       (Scr.VxMax + Scr.MyDisplayWidth) + n1;
  wy = (Scr.Vy + t->y) * (desk_h - m) /
       (Scr.VyMax + Scr.MyDisplayHeight) + m1;
  ...
  if (KeepMoving) {
    ...
    n1 = x2 * (Scr.VxMax +
	      Scr.MyDisplayWidth) / (desk_w * Scr.MyDisplayWidth);
    m1 = y2 * (Scr.VyMax +
	      Scr.MyDisplayHeight) / (desk_h * Scr.MyDisplayHeight);
    x = (x2 - n1) * (Scr.VxMax + Scr.MyDisplayWidth) / (desk_w - n) -
	Scr.Vx;
    y = (y2 - m1) * (Scr.VyMax + Scr.MyDisplayHeight) / (desk_h - m) -
	Scr.Vy;

    if (x + t->frame_width + Scr.Vx < 0) x = -Scr.Vx;
    if (y + t->frame_height + Scr.Vy < 0) y = -Scr.Vy;
	...
	(t->flags & STICKY)) {
      NewDesk = Scr.CurrentDesk - desk1;
      if (x > Scr.MyDisplayWidth - 16) x = Scr.MyDisplayWidth - 16;
      if (y > Scr.MyDisplayHeight - 16) y = Scr.MyDisplayHeight - 16;
      if (x + t->width < 16) x = 16 - t->width;
      if (y + t->height < 16) y = 16 - t->height;
    }
...
void IconMoveWindow(XEvent *Event, PagerWindow *t)
{
  ...
  if (KeepMoving) {
    ...
    y = y - y1;
    n1 = x * (Scr.VxMax +
	     Scr.MyDisplayWidth) / (icon_w * Scr.MyDisplayWidth);
    m1 = y * (Scr.VyMax +
	     Scr.MyDisplayHeight) / (icon_h * Scr.MyDisplayHeight);
