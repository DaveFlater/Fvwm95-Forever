...
// 2017-03-12 DWF
// This MUST be complete and harmonized with the ordering of the bitmap given
// in fvwm/module.h.  (It wasn't.)
char	*messages[MAX_MESSAGES+MAX_BUILTIN] =
{
	...
	"default_icon",
	"string",         // DWF added
	"function_end",   // DWF added
	"mini_icon",      // DWF added
	"scroll_region",  // DWF added
/* add builtins here */
	"startup",
	"shutdown",
	"unknown",
  ...
  while (buf != NULL)
      ...
      if (buf[0] == '*')
	{
	  ...
	  if (strncasecmp(buf, MyName, strlen(MyName)) == 0)
		...
		// 2017-04-15 DWF
		// Actually, allowing spaces would be good.
		// sound = strtok(NULL, " \t");
		sound = strtok(NULL, "\n");
	...
	while (1)
	{
				  ...
				  HEADER_SIZE*sizeof(unsigned long))) > 0)
		{
			...
			if(header[0] == START_FLAG)
			{
				...
				// 2017-03-12 DWF to find out what messages
				// were actually showing up here.  Not the
				// same stream that FvwmDebug shows.
